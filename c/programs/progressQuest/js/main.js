...
function ProgressBar(id, tmpl) {
  ...
  this.reposition = function (newpos) {
    ...
    if (this.bar) {
      ...
      this.bar.parent().find(".ui_progress__hint").text(game[this.id].hint);
    }
  };
  ...
  if (document) {
    ...
    $(window).unload(function (event) {
      ...
      if (storage.async) {
        ...
        // frequency.
        // else window.top.$alert("Game saved");
      }
    });
...
function quit() {
  ...
  SaveGame(function () { window.location.href = "index.php"; });
}
