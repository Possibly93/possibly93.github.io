...
function run() {
    ...
    var runner = function () {
        ...
        if (gamepads && gamepads[0]) {
            ...
            //if (buttonPressed(gp.buttons[3])) {joystick &= ~key /*console.log('down 3');*/ } else {/*console.log('up 3');*/};

            if (buttonPressed(gp.buttons[2])) {joystick &= ~16} else {joystick |= 16};
            if (buttonPressed(gp.buttons[7])) {joystick &= ~16} else {joystick |= 16};
            if (buttonPressed(gp.buttons[0])) {joystick &= ~32} else {joystick |= 32};
            if (buttonPressed(gp.buttons[5])) {joystick &= ~32} else {joystick |= 32};
...
/*function paintScreen() {
    ctx.putImageData(imageData, 0, 0); // 512, 384
    //ctx.scale(512, 384);

    //console.log(canvas);

    ctx.scale(canvas.width/512, canvas.height/384);
